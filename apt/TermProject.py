...
g_Tk = Tk()
g_Tk.geometry("400x620+450+100") # {width}x{height}+-{xpos}+-{ypos}

images = {'Mokoko': PhotoImage(file="images\모코코.gif"), 'CokeMo': PhotoImage(file="images\img.gif"),'MAPIMAGE': PhotoImage(file="images\지도.gif"),'EMAILIMAGE': PhotoImage(file="images\이메일.gif")}
...
# title 부분
    ...
    sendEmailButton = Button(frameTitle, font = fontNormal, image=images['EMAILIMAGE'], command = emailWindow)
   ...
   # sendEmailButton.bind('<Button-1>', emailWindow)
    ...
    image=images['MAPIMAGE'], command= mapClicked)
